...
| `FREE_FIRST_IMAGE_ENABLED` | Opens the free first image, which is gated on a confirmed email address. The post-signup screen tells new accounts that verifying unlocks it, so with this `false` that reason is aspirational. |
| `X_CLAIM_SERVICE_SECRET` | Shared secret for `POST /api/x-claims`, called by `x-dm-service` to park a photo sent by DM and mint a one-time claim link. Must match the value on the `x.pawsome3d.com` site byte for byte. Unset, the mint route answers 404 and the funnel stays dark. |
| `TRIPO_MULTIVIEW_MODEL_VERSION` | Pins the generator behind the reference left/back/right views. Unset, Tripo picks its current default, so two orders can be generated by different models and produce a view set that disagrees with itself. A guessed value fails the task outright, so leave it empty until the version you want is known. |
...
**There are two Node sites on this plan.** `pawsome3d.com` is the storefront
described below. `x.pawsome3d.com` runs `x-dm-service` and is deployed
separately — see [`x-dm-service/DEPLOY.md`](x-dm-service/DEPLOY.md). They share a
plan and a database but nothing else; the subdomain holds a credential that can
publish to a public timeline, which is why it is kept off the storefront.
