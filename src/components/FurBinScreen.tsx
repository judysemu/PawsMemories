...
function LegacyFurBinScreen({ creations, userProfile, onOpenCreditStore }: FurBinScreenProps) {
  ...
  const [pawprintPrintOrders, setPawprintPrintOrders] = useState<PawprintShopifyOrder[]>([]);
  ...
  return (
    <main className="mx-auto w-full max-w-7xl px-4 pb-28 pt-7 sm:px-6">
      ...
      {(filter === "all" || filter === "pawprints") && pawprintPrintOrders.length > 0 && <section className="mt-7">
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{pawprintPrintOrders.map((order) => <article key={order.id} className="rounded-[1.5rem] border border-white/30 bg-surface/75 p-4 shadow-lg backdrop-blur-2xl"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black text-on-surface">Shopify order #{order.id}</p><p className="mt-1 text-[11px] text-on-surface-variant">{new Date(order.created_at).toLocaleDateString()}</p></div><span className="rounded-full bg-primary/10 px-2 py-1 text-[10px] font-black uppercase text-primary">{order.status.replaceAll("_", " ")}</span></div><div className="mt-3 flex flex-wrap gap-2">{order.checkout_url && <a href={order.checkout_url} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-10 items-center justify-center rounded-xl bg-primary px-4 text-xs font-black text-on-primary">View on Shopify</a>}</div><p className="mt-3 text-[10px] text-on-surface-variant">Order status is set at checkout; tracking updates are not yet synced from Shopify.</p></article>)}</div>
      </section>}
