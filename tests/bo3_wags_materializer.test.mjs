  ...
  const pool = {
    async query(sql, params = []) {
      ...
      if (sql.includes("SET asset_status = 'generated'")) {
        ...
        if (row) {
          row.asset_status = "generated";
          row.asset_url = params[0];
          row.asset_error = null;
        }
        return [{ affectedRows: 1 }];
      }
...
async function transparentPngDataUrl() {
  const buf = await sharp({
    create: {
      ...
      height: 8,
      channels: 4,
      background: { r: 200, g: 80, b: 40, alpha: 0 },
    },
  })
    .png()
    .toBuffer();
...
const STICKER_ITEM = {
  slot: "sticker_1",
  title: "Leaf-Pile Zoomies",
  description: "Corgi mid-zoom",
};
const SEASONAL_ITEM = {
  slot: "seasonal",
  title: "Witch Hat",
  description: "October collectible",
};
const CREDIT_ITEM = {
  slot: "credit_pack",
  title: "20-Credit Boost",
  description: "Credits",
};
...
test("slot taxonomy: 2D-generative vs future-3D vs entitlement slots", () => {
  for (const slot of ["sticker_1", "sticker_5", "seasonal", "pawprint"])
    assert.equal(isGenerativeSlot(slot), true, slot);
  ...
  const prompt = buildSlotPrompt(
    {
      slot: "sticker_2",
      title: "Snack Gremlin",
      description: "Corgi eyeing a treat",
      colors: ["gold"],
    },
    {
      species: "dog",
...
test("pawprint and seasonal prompts carry theme and season, no transparency demand", () => {
  const ctx = {
    ...
    petName: null,
    season: "winter",
    theme: "Frost Whiskers",
  };
  const pawprint = buildSlotPrompt(
    { slot: "pawprint", title: "Snow Card", description: "Card art" },
    ctx,
  );
  assert.match(pawprint, /Frost Whiskers/);
  assert.match(pawprint, /a cat/);
  assert.doesNotMatch(pawprint, /TRANSPARENT/);
  const seasonal = buildSlotPrompt(
    { slot: "seasonal", title: "Icicle Crown", description: "Collectible" },
    ctx,
  );
...
test("materializes only generative slots, uploads assets, flips box delivered", async () => {
  const { pool, state } = makeFakePool({
    items: [STICKER_ITEM, SEASONAL_ITEM, CREDIT_ITEM, ACCESSORY_ITEM],
  });
  const calls = { generate: 0, upload: 0 };
  const png = await transparentPngDataUrl();
  const result = await materializeBoxAssets(
    pool,
    {
      generateImage: async () => {
        calls.generate += 1;
      ...
      uploadImage: async () => {
        calls.upload += 1;
        return `https://cdn.example/asset-${calls.upload}.png`;
      },
    },
    7,
  );
  ...
  assert.equal(
    state.items.find((i) => i.slot === "sticker_1").asset_url,
    "https://cdn.example/asset-1.png",
  );
  assert.equal(
    state.items.find((i) => i.slot === "credit_pack").asset_status,
    null,
  );
});
...
test("a failed slot does not block others and holds the box out of delivered", async () => {
  const { pool, state } = makeFakePool({
    items: [STICKER_ITEM, SEASONAL_ITEM],
  });
  const png = await transparentPngDataUrl();
  const result = await materializeBoxAssets(
    pool,
    {
      generateImage: async (prompt) =>
        prompt.includes("seasonal art card") ? null : png,
  ...
  assert.equal(
    result.delivered,
    false,
    "box must not deliver with a missing paid slot",
  );
  assert.equal(state.boxStatus, "approved");
  assert.equal(
    state.items.find((i) => i.slot === "seasonal").asset_status,
    "failed",
  );
  assert.match(
    state.items.find((i) => i.slot === "seasonal").asset_error,
...
test("an opaque sticker fails the real alpha gate and is marked failed", async () => {
  ...
  const result = await materializeBoxAssets(
    pool,
    {
      ...
      uploadImage: async () => {
        uploads += 1;
        return "https://cdn.example/should-not-exist.png";
      },
    },
    7,
  );
  ...
  assert.match(
    state.items[0].asset_error,
    /STICKER_NO_ALPHA_CHANNEL|STICKER_ALPHA_OPAQUE/,
  );
  assert.equal(state.boxStatus, "approved");
});
...
test("materializing an unapproved box is refused", async () => {
  ...
  await assert.rejects(
    () =>
      materializeBoxAssets(
        pool,
        { generateImage: async () => null, uploadImage: async () => "" },
        7,
      ),
    /not approved/,
  );
...
test("Plus materialization fails closed before any partial seven-call image run", async () => {
  ...
  assert.equal(
    state.items.every((item) => item.asset_status === null),
    true,
  );
});
...
function makeDeliveryPool() {
  ...
  const pool = {
    ...
    async query(sql, params = []) {
      ...
      if (sql.includes("FOR UPDATE")) return [[{ id: params[0] }]];
      if (sql.includes("COUNT(*) AS n")) return [[{ n: 0 }]];
      if (sql.includes("SELECT DISTINCT")) return [[]];
      if (sql.startsWith("INSERT INTO wardrobe_wags_box_items"))
        return [{ insertId: writes.length }];
      return [{ affectedRows: 1 }];
    },
    async getConnection() {
      return {
        ...
        async beginTransaction() {
          writes.push({ sql: "BEGIN", params: [] });
        },
        async commit() {
          writes.push({ sql: "COMMIT", params: [] });
        },
        async rollback() {
          writes.push({ sql: "ROLLBACK", params: [] });
        },
        release() {},
      };
    },
...
test("deliverBox with finalizeStatus:false creates items but never flips delivered", async () => {
  ...
  const result = await deliverBox(
    pool,
    { id: 9, user_phone: "+15550001111", plan_json: MINI_PLAN },
    { finalizeStatus: false },
  );
  assert.equal(result.itemsCreated, 2);
  assert.equal(
    pool.writes.some((w) => w.sql.includes("SET status = 'delivered'")),
    false,
  );
  const stickerInsert = pool.writes.find(
    (w) =>
      w.sql.startsWith("INSERT INTO wardrobe_wags_box_items") &&
      w.params[1] === "sticker_1",
  );
  assert.equal(
    stickerInsert.params[8],
    "pending",
    "generative slot starts pending",
  );
  const creditInsert = pool.writes.find(
    (w) =>
      w.sql.startsWith("INSERT INTO wardrobe_wags_box_items") &&
      w.params[1] === "credit_pack",
  );
  assert.equal(
    creditInsert.params[8],
    "none",
    "non-generative slot has no asset obligation",
  );
});

test("deliverBox default behavior still flips delivered (back-compat)", async () => {
  const pool = makeDeliveryPool();
  await deliverBox(pool, {
    id: 9,
