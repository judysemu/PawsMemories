...
test("the reference multiview call can pin a model version, and omits it when unset", () => {
  const tripo = fs.readFileSync(new URL("../tripo.ts", import.meta.url), "utf8");
  const fn = tripo.slice(
    tripo.indexOf("export async function startTripoImageToMultiview"),
    tripo.indexOf("function tripoMultiviewTaskId"),
  );
  // Every other Tripo submission pins its version so a provider-side bump
  // cannot silently change output. This one sent nothing, which is how one
  // order's views can be generated by a different model than the next.
  assert.match(fn, /TRIPO_MULTIVIEW_MODEL_VERSION/);
  assert.match(fn, /model_version/);
  // Unset must stay omitted: Tripo owns the valid version strings and a guessed
  // one fails the task, so the default has to remain today's behaviour.
  assert.match(fn, /\?\s*\{\s*model_version[\s\S]*?\}\s*:\s*\{\s*\}/);
});
