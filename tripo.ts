...
export interface TripoMultiviewOptions {
  /**
   * Pins the generator behind the left/back/right views. Every other Tripo call
   * in this file pins its version so a provider-side bump cannot change output
   * under us; this one sent nothing, so Tripo chose its current default and two
   * orders minutes apart could be generated by different models. That shows up
   * as a view set that disagrees with itself -- a front that reads as one breed
   * and a rear that reads as another.
   *
   * Left undefined it is omitted entirely, preserving today's behaviour: the
   * valid version strings are Tripo's to publish, and sending a guessed one
   * fails the task outright. Set TRIPO_MULTIVIEW_MODEL_VERSION once the version
   * you want is known and every future reference set is reproducible.
   */
  modelVersion?: string;
}

export async function startTripoImageToMultiview(
  imageBytes: Buffer,
  opts?: TripoMultiviewOptions,
): Promise<string> {
  assertExternalGenerativeProviderAllowed("tripo");
  const uploaded = await uploadBytesToTripo(imageBytes);
  // Same rule as startImageTo3D's `common`: only send a product choice someone
  // actually made, so an unset knob keeps Tripo's adaptive default.
  const modelVersion = opts?.modelVersion || process.env.TRIPO_MULTIVIEW_MODEL_VERSION || "";
